...
namespace UsuariosAPI.Services
{
    public class LoginService
    {
        ...
        public Result SolicitaResentSenhaUsuario(SolicitaResetRequest request)
        {
            CustomIdentityUser identityUser = RecuperaUsuarioPorEmail(request.Email);
            if (identityUser != null)
            {
                string codigoDeRecuperaçao = _signInManager.UserManager.GeneratePasswordResetTokenAsync(identityUser).Result;
                return Result.Ok().WithSuccess(codigoDeRecuperaçao);
            }
            return Result.Fail("Falha ao solcitar redefinição");
        }
